...
vector<vector<double>> matrix_solver(vector<vector<double>>S,vector<double> lf, int feed_stage){
        vector<vector<double>>l;
        int no_of_comp=S.size()-1;
        int no_of_stages=S[0].size()-1;
    for(int i=1;i<=no_of_comp;i++)
    {
      vector<vector<double>> A=calculate_Ai(S,i);
      vector<double>B(no_of_stages,0);
      B[feed_stage-1]=-lf[i];
      // matrix solver
      vector<double> l1;
      try
